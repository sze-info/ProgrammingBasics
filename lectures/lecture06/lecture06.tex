...
%~ %14
%~ \begin{frame}{Mintapélda függvények használatára}
...
%~ %18
%~ \begin{frame}{Fogalmak -- élettartam}
...
%~ \begin{frame}{Fogalmak -- élettartam}
  %~ \begin{columns}[T]
    %~ \column{0.5\textwidth}
      ...
      %~ \item Lokális
      %~ \begin{itemize}
        %~ \item blokkba belépéstől annak elhagyásáig rendelnek memóriát hozzájuk
        %~ \item függvényparaméterek, vezérlési szerkezetek definíciói is ilyenek
    ...
    %~ \column{0.5\textwidth}
      %~ \begin{exampleblock}{\textattachfile{nk1.c}{nk1.c}}
        %~ \tiny
        %~ \lstinputlisting[style=c,firstline=17,lastline=25,numbers=right,firstnumber=17]{nk1.c}
      %~ \end{exampleblock}
  %~ \end{columns}
  %~ \vfill
  %~ \scriptsize
  ...
  %~ \begin{description}[]
    ...
    %~ \item[$n$] 3 fv. hívás miatt 3x létrejön a fv. hívásakor/megszűnik visszatéréskor (18 v. 23. sor)
    %~ \item[$f$] létrejön ha $n$ elég nagy, és megszűnik amikor a végrehajtás a 23. sorhoz ér
    %~ \item[$i$] a 20. sor elérése pillanatában jön létre, és a ciklusból kilépéskor felszabadul
  %~ \end{description}
%~ \end{frame}

%~ %20
%~ \begin{frame}{Fogalmak -- hatáskör}
  %~ \kiemel{Hatáskör} (érvényességi tartomány, hatókör, scope): meghatározza, hogy az objektumot a program \emph{mely részén} 
%~ lehet elérni;
%~ deklarációtól és annak helyétől függően:
  %~ \begin{itemize}
