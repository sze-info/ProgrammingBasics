...
bool check(const struct date* d) { // content validation
...
int dayOfYear(const struct date* d) { // determining the day of the year
...
int base(const struct date* d) { // days elapsed since 01.01.0000
  int b = 0;
...
int difference(const struct date* begin, const struct date* end) { // days elapsed
...
struct date monthAndDay(int year, int dayOfYear) { 
  struct date d = { dayOfYear, 0, year };
  int day;
  for(d.month=1; d.day>(day=daysOfMonth(year, d.month)); d.month++) {
    d.day -= day;
  }
  return d;
}
